...
class ValueCalculator(tk.Tk):
    ...
    def _build_ui(self):
        ...
        operation_menu = ttk.Combobox(
            ...
            values=("Sum", "Average", "Minimum", "Maximum", "Product"),
            state="readonly",
        )
    ...
    def calculate(self):
        ...
        result = self._calculate_result(calculation, values)

        self.result_text.set(
            f"{calculation}: {self._format_number(result)}\n\n"
            f"Values used: {len(values)}\n"
            f"Total: {self._format_number(sum(values))}\n"
            f"Average: {self._format_number(sum(values) / len(values))}\n"
            f"Minimum: {self._format_number(min(values))}\n"
            f"Maximum: {self._format_number(max(values))}"
        )
    ...
    @staticmethod
    def _calculate_result(calculation, values):
        match calculation:
            case "Average":
